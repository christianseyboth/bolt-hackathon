...
const getFormatIcon = (format: string) => {
    switch (format) {
        case 'pdf':
            return <IconFileTypePdf className='h-4 w-4 text-red-400' />;
        case 'csv':
            return <IconFileTypeCsv className='h-4 w-4 text-green-400' />;
        case 'xlsx':
            return <IconFileTypeXls className='h-4 w-4 text-blue-400' />;
        default:
            return <IconFileText className='h-4 w-4 text-neutral-400' />;
    }
};
                    ...
                    setReports((prev) =>
                        prev.map((report) =>
                            report.id === payload.new.id
                                ? (payload.new as ReportHistoryItem)
                                : report
                        )
                    );
                }
                                ...
                                {currentReports.map((report) => (
                                    ...
                                    >
                                        ...
                                        <div className='flex items-center gap-2 ml-4'>
                                            ...
                                            >
                                                {isDeleting &&
                                                deleteModal.reportId === report.id ? (
                                                    <IconLoader2 className='h-4 w-4 animate-spin' />
                                                ) : (
