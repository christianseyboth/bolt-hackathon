...
const getStatusColor = (status: string) => {
    switch (status) {
        ...
        case 'quarantined': return 'bg-amber-500/20 text-amber-400 border-amber-500/30';
        case 'flagged': return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="block md:hidden space-y-3">
                            {mockThreats.map((threat) => (
                                <div key={threat.id} className="p-3 bg-neutral-800/30 rounded-lg border border-neutral-700">
                                    <div className="flex items-start justify-between mb-2">
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-sm font-medium text-neutral-200 truncate">
                                                {threat.subject}
                                            </h4>
                                            <p className="text-xs text-neutral-400 truncate">{threat.sender}</p>
                                        </div>
                                        <Badge className={`ml-2 text-xs ${getRiskColor(threat.riskLevel)}`}>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-neutral-500">{threat.threatType}</span>
                                        </div>
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody className="space-y-2">
                                        {mockThreats.map((threat, index) => (
                                            <tr key={threat.id} className={index !== mockThreats.length - 1 ? 'border-b border-neutral-800' : ''}>
                                                <td className="py-3 pr-4">
                                                    <div className="flex items-center gap-2">
                                                        <IconMail className="h-4 w-4 text-neutral-400 flex-shrink-0" />
                                                        <span className="text-sm text-neutral-200 truncate max-w-[200px]" title={threat.subject}>
                                                            {threat.subject}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td className="py-3 pr-4">
                                                    <span className="text-sm text-neutral-400 truncate max-w-[150px]" title={threat.sender}>
                                                        {threat.sender}
                                                    </span>
                                                </td>
                                                <td className="py-3 pr-4">
                                                    <Badge className={`text-xs ${getRiskColor(threat.riskLevel)}`}>
                                                        {threat.riskLevel.toUpperCase()}
                                                    </Badge>
                                                </td>
                                                <td className="py-3 pr-4">
                                                    <span className="text-sm text-neutral-300">{threat.threatType}</span>
                                                </td>
                                                <td className="py-3 pr-4">
                                                    <Badge variant="outline" className={`text-xs ${getStatusColor(threat.status)}`}>
                                                        {threat.status}
                                                ...
                                                <td className="py-3 pr-4">
                                                    <span className="text-sm text-neutral-400">{threat.detectedAt}</span>
                                                </td>
                                                <td className="py-3">
                                                    <Button variant="ghost" size="sm">
                                                        <IconEye className="h-4 w-4" />
