  ...
  return (
    <div className="w-full relative h-full pt-20 md:pt-40">
      <div className="px-6">
        ...
        <Subheading>
          From individual professionals to enterprise teams - SecPilot provides scalable email security
          solutions with advanced threat detection, comprehensive dashboards, and seamless integrations.
          Protect against phishing, malware, ransomware, and business email compromise with plans
          starting at $19/month for small businesses and growing to enterprise-grade protection.
        </Subheading>
      </div>
    ...
    <div className="w-full h-100 bg-gradient-to-br from-zinc-800 to-zinc-900 rounded-lg p-4 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-emerald-400 font-medium">Security Command Center</span>
        </div>
      ...
      <div className="bg-zinc-800/30 rounded p-3 border border-zinc-700">
        <div className="text-sm text-neutral-300 font-medium mb-2">Recent Threat Detections</div>
        <div className="space-y-2">
          {[
            { type: "Phishing", sender: "fake-bank@suspicious.com", time: "2 min ago", severity: "high" },
            { type: "Malware", sender: "invoice@malicious.net", time: "5 min ago", severity: "critical" },
            { type: "Spam", sender: "promo@spammer.org", time: "8 min ago", severity: "low" },
            { type: "Scam", sender: "hacker@scammer.org", time: "12 min ago", severity: "critical" },
          ].map((threat, i) => (
            <motion.div
              key={i}
    ...
    <div className="w-full h-80 bg-gradient-to-br from-zinc-800 to-zinc-900 rounded-lg p-4 relative overflow-hidden">
      ...
      <div className="space-y-4">
        <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
          <div className="text-sm text-neutral-300 font-medium mb-3">REST API Endpoints</div>
          <div className="space-y-2">
            {[
              { method: "GET", endpoint: "/api/v1/threats", desc: "Fetch threat data", status: "Active" },
              { method: "POST", endpoint: "/api/v1/scan", desc: "Submit email for scanning", status: "Active" },
              { method: "GET", endpoint: "/api/v1/reports", desc: "Generate security reports", status: "Active" },
            ].map((api, i) => (
              <div key={i} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-neutral-300 font-mono">{api.endpoint}</div>
        ...
        <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
          <div className="text-sm text-neutral-300 font-medium mb-3">Webhook Configuration</div>
          <div className="space-y-2">
            {[
              { name: "Slack Alerts", url: "https://hooks.slack.com/...", events: "High threats", status: "Active" },
              { name: "SIEM Integration", url: "https://siem.company.com/...", events: "All events", status: "Active" },
              { name: "Teams Notifications", url: "https://outlook.office.com/...", events: "Critical only", status: "Pending" },
            ].map((webhook, i) => (
              <div key={i} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-neutral-500 font-mono text-xs">{webhook.url.substring(0, 25)}...</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`text-xs ${webhook.status === 'Active' ? 'text-emerald-400' : 'text-amber-400'}`}>
                    {webhook.status}
                  </div>
        ...
        {/* Integration Status */}
        <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
          <div className="text-sm text-neutral-300 font-medium mb-2">Integration Status</div>
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={i} className="bg-zinc-800/30 rounded p-2 text-center">
                <div className="text-xs text-neutral-300 font-medium">{integration.name}</div>
                <div className={`text-xs ${integration.status === 'Connected' ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {integration.status}
                </div>
                <div className="text-xs text-neutral-500">{integration.requests}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="w-full h-80 bg-gradient-to-br from-zinc-800 to-zinc-900 rounded-lg p-4 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button className="text-xs bg-zinc-700 text-neutral-300 px-2 py-1 rounded">Weekly</button>
          <button className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded border border-emerald-500/30">Monthly</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
          ...
          <div className="text-sm text-neutral-300">Security posture improved by 23% this month</div>
        </div>
        <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
