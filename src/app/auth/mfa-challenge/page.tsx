...
export default function MFAChallenge() {
  ...
  const handleVerifyMFA = async (e: React.FormEvent) => {
    ...
    if (!isOAuth && !password) {
      ...
      setError('For security, please re-enter your password along with your authenticator code');
      return;
    }
        ...
        <CardHeader className="text-center">
          ...
          <CardDescription>
            ...
            {email && (
              <span className="block mt-2 font-medium text-foreground">
                Signing in as: {email}
                {isOAuth && <span className="block text-sm text-muted-foreground">(OAuth Account)</span>}
              </span>
            )}
          </CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleVerifyMFA} className="space-y-4">
            ...
            <div className="space-y-2">
              <Button
                type="submit"
                disabled={isLoading || code.length !== 6 || (showPasswordInput && !isOAuth && !password)}
                className="w-full"
              >
                {isLoading ? (
                  <>
